...
df=pd.read_excel('D:/research/new_model_May/.idea/5-20_data_for_plos_paper_nounknown.xlsx') # this time use absolute address to input data
...
#X_new=pd.get_dummies(df, columns=['gender','hyperlipidaemia','obesity','hypertension','room_space','week_work_hours','spense','age','health_awareness','insurance','stress'])
X_new=X_new.to_numpy()
#X_new=np.transpose(X_new)
X_new=np.float32(X_new)
y=df['diabetes'].values
# #split data to training data and test data
trX, trY, teX, teY= train_test_split(X_new,y, test_size=0.30,
                                                    random_state=100)
print(type(X_new))
print(X_new.dtype)
print(X_new.size)
...
# bulid DBN
RBM_hidden_sizes = [19, 19, 1 ] #create 4 layers of RBM with size 785-500-200-50
